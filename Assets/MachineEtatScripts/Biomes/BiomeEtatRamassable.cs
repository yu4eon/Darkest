...
/// <summary>
/// Classe pour l'état cultivable du biome
/// qui permet de changer l'état du biome en final
/// selon une séquence de transitions
/// </summary>
public class BiomeEtatRamassable : BiomesEtatsBase
{
    Material _matCube; // Matériel pour le biome
    float _posyInit; // Position y initiale du biome
    /// <summary>
    ...
    public override void InitEtat(BiomesEtatsManager biome)
    {
        ...
        {
            biome.StartCoroutine(ActiverItem(biome));
            // biome.infos["item"].SetActive(true);
        }
        _posyInit = biome.transform.localPosition.y; // sauvegarde la position y initiale du biome
        // biome.StartCoroutine(ChangerEtat(biome)); // Commence la coroutine de changement d'état
    }
    /// <summary>
    /// Sortie de l'état
    /// </summary>
    ...
    public override void ExitEtat(BiomesEtatsManager biome)
    {
        ...
        biome.StopAllCoroutines(); // arrête toutes les coroutines
        // _matCube = (Material)Resources.Load("etats/matActivable"); // charge le matériel du cube pour celui du biome
        // biome.GetComponent<Renderer>().material = _matCube; // applique le matériel au cube
        // biome.transform.localPosition = new Vector3(biome.transform.localPosition.x, _posyInit, biome.transform.localPosition.z); // réinitialise la position y du biome
        // if (biome.infos["item"] != null)
        // {
    ...
    IEnumerator ActiverItem(BiomesEtatsManager biome)
    {
        biome.infos["item"].SetActive(true);
        Vector3 scaleInit = biome.infos["item"].transform.localScale;
        biome.infos["item"].transform.localScale = new Vector3(0, 0, 0);
        while (biome.infos["item"].transform.localScale.x < scaleInit.x)
        {
            biome.infos["item"].transform.localScale = Vector3.Lerp(biome.infos["item"].transform.localScale, scaleInit, Time.deltaTime * biome.infos["vitesseApparition"]);
            yield return null;
        }
        biome.StopCoroutine(ActiverItem(biome));
        yield break;
